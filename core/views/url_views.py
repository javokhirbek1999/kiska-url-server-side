...
class RedirectApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        short_url_object = ShortURL.objects.get(url_hash=kwargs.get('pk'))
        all_short_url_instance = AllShortURL.objects.get(url=short_url_object.short_url)
        all_short_url_instance.visited += 1
        all_short_url_instance.save()
        return redirect(short_url_object.original_url.url)
